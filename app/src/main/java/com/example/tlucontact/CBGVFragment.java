    ...
    public class CBGVFragment extends Fragment {
        ...
        private List<CBGV> filteredList;
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
            ...
            cbgvAdapter = new CBGVAdapter(filteredList, new CBGVAdapter.IClickItemCBGV() {
                @Override
                public void onClickItemCBGV(CBGV cbgv) {
                    Intent intent = new Intent(requireContext(), DetailCBGVActivity.class);
                    intent.putExtra("cbgv_data", cbgv);
        ...
        public void filterList(String text) {
            if (filteredList != null) {  // Kiểm tra danh sách trước khi sử dụng
                filteredList.clear();
            }
            if (text.isEmpty()) {
                filteredList.addAll(listCB);
                ...
                for (CBGV cbgv : listCB) {
                    if (cbgv.getTenCB().toLowerCase().contains(text.toLowerCase())) {
                        filteredList.add(cbgv);
                    }
                }
            }
            cbgvAdapter.notifyDataSetChanged();
        }
    }